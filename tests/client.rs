...
use rand::RngCore;
use rand::rngs::OsRng;
use reqwless::client::{HttpClient, TlsCipherSuiteDefault};
...
async fn test_request_response_notls() {
    ...
    let mut client: HttpClient<'_, _, _, TlsCipherSuiteDefault> = HttpClient::new(&TCP, &LOOPBACK_DNS);
...
async fn test_resource_rustls() {
    ...
    let mut client: HttpClient<'_, _, _, TlsCipherSuiteDefault> = HttpClient::new_with_tls(
        &TCP,
        &LOOPBACK_DNS,
        TlsConfig::new(
            OsRng.next_u64(),
...
#[tokio::test]
#[cfg(feature = "embedded-tls")]
async fn test_resource_rustls_configured_cipher() {
    use reqwless::client::{TlsConfig, TlsVerify};
    ...
    // Only enable the cipher suite configured on the client side, so that a successful
    // handshake guarantees the cipher configured through the generic was negotiated.
    let config = rustls::ServerConfig::builder()
    ...
    let mut client: HttpClient<'_, TokioTcp, LoopbackDns, embedded_tls::Aes256GcmSha384> = HttpClient::new_with_tls(
        &TCP, 
        &LOOPBACK_DNS, 
        TlsConfig::new(
            OsRng.next_u64(),
...
fn compile_tests() {
    ...
    async fn rx_buffer_lifetime_is_propagated_to_output<'buf>(port: u16, rx_buf: &'buf mut [u8]) -> &'buf mut [u8] {
        let mut http: HttpClient<'_, _, _, TlsCipherSuiteDefault> = HttpClient::new(&TCP, &LOOPBACK_DNS);
